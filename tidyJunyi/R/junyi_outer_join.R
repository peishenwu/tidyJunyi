#' A workable alternative for dplyr::outer_join to use in junyi bigquery datasets
#' @param x tbl object
#' @param y tbl object
...
junyi.outer_join <- function(x, y, by = NULL, by.x = NULL, by.y = NULL, restore.names = T,
                             tablename = NULL){
  ...
  if(length(by.x)==1 & length(by.y)==1){
    join.sql <- paste("SELECT * FROM (",x.sql,") AS pre OUTER JOIN (",y.sql,") AS post ON pre.",by.x," = post.",by.y,sep="")
  }else{
    #get colnames
    ...
    join.sql <- paste("SELECT * FROM (",x.sql,") AS pre OUTER JOIN (",y.sql,") AS post ON pre.",keyname," = post.",keyname,sep="")
  }#end if
  ##
