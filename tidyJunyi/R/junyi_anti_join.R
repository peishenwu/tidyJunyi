#' A workable alternative for dplyr::anti_join to use in junyi bigquery datasets
#' @description return all rows from x where there are not matching values in y, keeping just columns from x
#' @param x tbl object
#' @param y tbl object
...
junyi.anti_join <- function(x, y, by = NULL, by.x = NULL, by.y = NULL, tablename = NULL){
  ...
  join.sql <- paste("SELECT * FROM (",x.sql,")
                     WHERE ",by.x," NOT IN (",paste("SELECT ",by.y," FROM (",y.sql,")",sep=""),")",sep="")
